...
def run(server_class=HTTPServer, handler_class=Resquest, port=8080,logname="access"):
    logging.basicConfig(level=logging.INFO,filename=logname+".log",filemode="a",format='%(message)s')
...
if __name__ == '__main__':
    ...
    if len(argv) > 2:
        run(port=int(argv[1]),logname=str(argv[2]))
    else:
        run()